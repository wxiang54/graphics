...
void draw_polygons( struct matrix *polygons, screen s, color c ) {
  ...
  for (point=0; point < polygons->lastcol-1; point+=3) {
    ...
    if (Nz <= 0) continue; //don't draw invisible things!
...
void add_sphere( struct matrix * polygons, 
  ...
  int n = (int)(1 / step) + 1; //num of points per slice
  
  int i;
  for (i=0; i < (points->lastcol); i++) {
    if (i%n >= n-1) continue; //last iteration not needed
...
void add_torus( struct matrix * polygons, 
  ...
  int n = (int)(1 / step); //num of points per slice
  ...
  for (i=0; i < (points->lastcol); i++) {
	...
	add_polygon( polygons,
      ...
      if (i%n < n-1) { //arbitrary point in last slice
	add_polygon( polygons,
		     (points->m)[0][i], (points->m)[1][i], (points->m)[2][i],
		     (points->m)[0][i%n], (points->m)[1][i%n], (points->m)[2][i%n],
		     (points->m)[0][i+1], (points->m)[1][i+1], (points->m)[2][i+1]);
